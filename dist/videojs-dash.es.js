...
var Html5DashJS = /*#__PURE__*/function () {
  function Html5DashJS(source, tech, options) {
    ...
    setupTextTracks.call(null, this.player, tech, options); // Setup quality levels

    this.setupQualityLevels_(); // Attach the source with any protection data
  ...
  */
  ...
  _proto.setupQualityLevels_ = function setupQualityLevels_() {
    ...
    if (player && player.qualityLevels) {
      ...
      this.mediaPlayer_.on(dashjs.MediaPlayer.events.PLAYBACK_METADATA_LOADED, function () {
        _this2.videoRates_ = _this2.mediaPlayer_.getBitrateInfoListFor('video');
        _this2.audioRates_ = _this2.mediaPlayer_.getBitrateInfoListFor('audio');
        var normalizeFactor = _this2.videoRates_[_this2.videoRates_.length - 1].bitrate;
        _this2.audioMapper_ = _this2.videoRates_.map(function (rate) {
          return Math.round(rate.bitrate / normalizeFactor * (_this2.audioRates_.length - 1));
        });

        _this2.videoRates_.forEach(function (vrate) {
          _this2.qualityLevels_.addQualityLevel({
            id: vrate.bitrate,
            width: vrate.width,
            height: vrate.height,
      ...
      this.qualityLevels_.on('change', function (event) {
        ...
        var autoSwitchingEnabled = _this2.mediaPlayer_.getSettings().streaming.abr.autoSwitchBitrate;

        if (enabledQualities.length === 1) {
          if (autoSwitchingEnabled.video) {
            _this2.mediaPlayer_.updateSettings({
              'streaming': {
                'abr': {
                  'autoSwitchBitrate': {
                    'video': false,
          ...
          _this2.mediaPlayer_.setQualityFor('video', event.selectedIndex);

          _this2.mediaPlayer_.setQualityFor('audio', _this2.audioMapper_[event.selectedIndex]);
      ...
      this.mediaPlayer_.on(dashjs.MediaPlayer.events.QUALITY_CHANGE_REQUESTED, function (event) {
        if (event.mediaType === 'video') {
          _this2.qualityLevels_.selectedIndex_ = event.newQuality; // this.qualityLevels_.trigger({
